 ...
 */ 
class raw_dim_institution(huemulBigDataGov: huemul_BigDataGovernance, Control: huemul_Control) extends huemul_DataLake(huemulBigDataGov, Control) with Serializable  { 
   this.Description = "Instituciones" 
   ...
   CurrentSetting.FileName = "GDD_M_SGT_Instituciones_{{YYYY}}{{MM}}.dat" 
  ...
  def open(Alias: String, ControlParent: huemul_Control, ano: Integer, mes: Integer, dia: Integer, hora: Integer, min: Integer, seg: Integer): Boolean = { 
    ...
    try {  
      ...
      val rowRDD = this.DataRDD 
            ...
            .filter { x => x.length()>=4 && huemulBigDataGov.isAllDigits(x.substring(0, 1) )  } 
 ...
 */ 
object raw_dim_institution_test { 
/** 
  ...
  def main(args : Array[String]) { 
    ...
    val DF_RAW =  new raw_dim_institution(huemulBigDataGov, Control) 
