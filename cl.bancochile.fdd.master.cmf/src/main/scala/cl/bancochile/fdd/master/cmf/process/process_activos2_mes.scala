  ...
  def process_master(huemulBigDataGov: huemul_BigDataGovernance, ControlParent: huemul_Control, param_year: Integer, param_month: Integer, param_day: Integer): Boolean = { 
    ...
    try {              
      ...
      Control.NewStep("Generar Lógica de negocio") 
       ...
       //  Df1.DF_from_SQL("DF_TEMPORAL","""SELECT * FROM raw_activos2_mes""") 
           ...
           var Df2 = raw_activos2_mes.DataFramehuemul.DataFrame.withColumn("idx",org.apache.spark.sql.functions.monotonically_increasing_id()).withColumn("periodo_mes",org.apache.spark.sql.functions.lit(s"""$proc_date_new"""))
...
//  huemulTable_tbl_activos2_mes.DF_from_SQL("TBL_ACTIVOS2_mes","""SELECT * FROM df_final""") 
//huemulTable_tbl_activos2_mes.DataFramehuemul.DataFrame.select("idx","Institucion","periodo_mes").show()
   	    huemulTable_tbl_activos2_mes.DF_from_SQL("TBL_ACTIVOS2_messys",s"""SELECT row_number() over (order by idx) as id_interfaz,* FROM df_final""")	 
	  ...
	  huemulTable_tbl_activos2_mes.Act_cred_cpcac_pers_cons_total.SetMapping("Act_cred_cpcac_pers_cons_total")
        ...
        // huemulTable_tbl_activos2_mes.DataFramehuemul.DataFrame.count() 
